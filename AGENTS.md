...
Use `chrome-devtools` through `scripts/codex-chrome-devtools-mcp.sh` for workspace browser automation. The policy default is isolated and headless; shared or headful modes are exception paths for shared auth or visible debugging only. Keep one working tab by default, stay under the three-tab cap, and do not use ad-hoc browsers for chat-driven browsing. Actual MCP defaults belong in tracked `.codex/config.toml` and the wrapper scripts, not in prompt prose.
