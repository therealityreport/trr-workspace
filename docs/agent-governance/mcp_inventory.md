...
Project-scoped custom agents under `.codex/agents/` may declare narrower MCP surfaces for their own runs. Those agent-local overrides do not replace the user-global or project-global MCP inventory; they narrow access for the spawned agent. User-level, system-level, and plugin skills remain inherited by default; TRR does not provide project-local canonical skills and must not suppress unrelated user-owned capabilities.
