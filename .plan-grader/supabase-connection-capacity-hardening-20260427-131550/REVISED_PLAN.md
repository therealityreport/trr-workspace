...
Use these as the executor's short-horizon success metrics, not just as implementation outputs:

1. Default `make dev` and normal admin browsing no longer produce repeated local `EMAXCONNSESSION` events during a focused smoke pass across `/admin/social`, one social profile, one season/week social page, and one non-social Supabase-backed admin page.
2. Workspace preflight reports local holder budget at or below the documented normal-profile target, with unsafe ignored env overrides failing under `WORKSPACE_ENFORCE_DB_HOLDER_BUDGET=1`.
3. App request-time DDL count is `0` for app repositories and route handlers after Phase 1/8B.
4. The `14` high-fanout production-risk app direct-SQL call sites are either migrated behind backend APIs or tracked with an owner, exception reason, and follow-up date.
5. `docs/workspace/api-migration-ledger.md` no longer contains only placeholder rows for routes changed by this plan.
6. Production capacity rows are populated with live values or explicit blocked-owner rows before any Supavisor pool-size change.
7. Production Vercel env review proves the runtime does not read retained legacy `DATABASE_URL`, `POSTGRES_*`, or `SUPABASE_*` values as primary DB credentials.

## execution_routing

Use inline execution for Phases 0-2 because they share workspace contracts and should land as one coherent baseline. After those guardrails are merged, use `orchestrate-subagents` only for independent, disjoint workstreams:

- Backend API workstream: Phase 5 aggregate endpoints and backend tests.
- App compatibility/fallback workstream: Phase 5 app route follow-through plus Phase 8 UI/error-envelope tests.
- Workspace/ops workstream: Phase 6 health pressure snapshots, Phase 9 Vercel/env guards, and Phase 10 capacity evidence.
- DB/schema workstream: Phase 7 query/index evidence plus Phase 8B migration ownership.

Do not parallelize edits that touch the same files, migration numbering, or shared env contract docs. Use backend-first sequencing for any API or schema contract change, then app follow-through in the same session.
...
## stop_conditions_and_human_checks

Stop and ask before:

1. Removing or mutating production Vercel env vars, even if the plan says they are retained legacy values.
2. Raising Supavisor pool size or changing Supabase compute tier.
3. Applying destructive or irreversible migrations.
4. Removing `admin` or `core` from local PostgREST exposure without a confirmed replacement path for local admin workflows.
5. Quarantining app migrations when parity has not been proven against a disposable or branch database.
6. Broadly switching app/backend traffic to transaction mode before the one-route flight test is documented.

If Supabase advisors, production Dashboard settings, or Vercel env access remain permission-blocked, record the blocker and owner in the relevant snapshot doc instead of inventing values.

## ADDITIONAL SUGGESTIONS

These suggestions are now accepted plan work. Execute them after their dependent core phases, not as a detached cleanup list.

### Suggestion 1 - Add a one-page operator runbook

Source: Suggestion 1, "Add a one-page operator runbook"

Concrete changes:

- Add `docs/workspace/db-pressure-runbook.md` or a compact section in `docs/workspace/dev-commands.md`.
- Include the exact operator flow for "admin page timed out" or `EMAXCONNSESSION`:
  - check `make preflight`;
  - inspect app/backend logs;
  - run holder snapshot/status command;
  - identify app/backend/worker lane pressure;
  - decide whether to retry, switch profile, disable worker mode, or escalate.
- Link the runbook from `docs/workspace/supabase-capacity-budget.md` and `docs/workspace/dev-commands.md`.

Dependencies:

- Phase 0 baseline commands.
- Phase 6 DB pressure endpoint if available; otherwise use script-only holder snapshots.
...
- Run the documented commands from a normal local shell.
- Confirm every referenced path/command exists.
- Confirm the runbook does not include secrets or full DB URLs.

Acceptance criteria:

- An operator can diagnose the next admin timeout without reading the full implementation plan.
- The runbook has one clear branch for local dev and one branch for production/preview evidence capture.

Commit boundary:

- One docs-only commit after Phase 0/6 commands are stable.

### Suggestion 2 - Add a fixture-backed fake `pg_stat_activity` test

Source: Suggestion 2, "Add a fixture-backed fake `pg_stat_activity` test"

Concrete changes:

- Add fixture data representing grouped `pg_stat_activity` rows by `application_name`, role, state, client address, and holder count.
- Test the parser/formatter that powers workspace status output and any backend `/health/db-pressure` summary.
- Include edge cases:
  - empty result set;
  - unknown `application_name`;
  - multiple holders for the same lane;
  - permission-blocked or unsupported query path.

Dependencies:

- Phase 0 holder snapshot shape.
- Phase 6 health pressure endpoint or script output format.

Affected surfaces:

- workspace script tests, likely under `scripts/` test files;
- backend health endpoint tests if Phase 6 exposes grouped holder data;
- holder snapshot fixtures under an existing test fixture path.

Validation:

- Run the targeted script/backend tests that consume the fixture.
- Verify fixture tests pass without a live Supabase connection.

Acceptance criteria:

- Holder snapshot formatting has deterministic tests.
- Permission-blocked snapshots produce a stable unsupported/blocked response instead of failing the whole health surface.

Commit boundary:

- One test/tooling commit paired with Phase 6.

### Suggestion 3 - Add a plan-owned glossary for connection terms

Source: Suggestion 3, "Add a plan-owned glossary for connection terms"

Concrete changes:

- Add a short glossary defining:
  - Supavisor session mode;
  - Supavisor transaction mode;
  - direct SQL;
  - Supabase REST/Auth/Storage API;
  - service role;
  - anon/public key;
  - Vercel attached pool;
  - app direct SQL;
  - backend aggregate endpoint;
  - holder budget.
- Link the glossary from the env contract and capacity docs.
- Use the glossary terms consistently in future plan/docs changes.
...
- `docs/workspace/supabase-capacity-budget.md`
- `docs/workspace/env-contract.md`
- optional `docs/workspace/supabase-glossary.md`

Validation:

- `rg` for the glossary links and major terms in workspace docs.
- Manual review that definitions do not conflict with Supabase's connection-mode behavior.

Acceptance criteria:

- New contributors can distinguish Postgres holders from Supabase API calls without reading source code.
- The glossary is referenced by both env and capacity docs.

Commit boundary:

- One docs-only commit with Phase 2.

### Suggestion 4 - Add a migration numbering policy

Source: Suggestion 4, "Add a migration numbering policy"

Concrete changes:

- Add a lightweight migration naming/ordering policy:
  - preferred timestamp or sequence format;
  - no duplicate numeric prefixes unless ordering is explicitly documented;
  - backend-owned shared schema changes live under `TRR-Backend/supabase/migrations`;
  - app migrations must be app-local or allowlisted.
- Link this policy from Phase 8B migration ownership docs.
- Update the migration ownership lint documentation to mention the policy.

Dependencies:

- Phase 8B duplicate-prefix cleanup.

Affected surfaces:

- `TRR-Backend/supabase/migrations/README.md` if added;
- `docs/workspace/dev-commands.md`;
- `docs/workspace/app-migration-ownership-allowlist.txt`;
- `scripts/migration-ownership-lint.py` documentation/help text if appropriate.

Validation:

- `python3 scripts/migration-ownership-lint.py`
- Manual check that duplicate-prefix policy matches the actual renamed/consolidated migrations.

Acceptance criteria:

- Future migrations have one documented ordering convention.
- Duplicate prefix exceptions require explicit documentation rather than accidental tolerance.

Commit boundary:

- One docs/tooling commit with Phase 8B.

### Suggestion 5 - Add a local pressure rehearsal command

Source: Suggestion 5, "Add a local pressure rehearsal command"

Concrete changes:

- Add a command such as `make db-pressure-rehearsal` or a workspace script wrapper.
- The command should:
  - confirm local stack/profile;
  - open or request the same read surfaces used in manual validation where practical;
  - sample holder snapshots before/during/after;
  - record app/backend timeout and `EMAXCONNSESSION` signals;
  - write a redacted local artifact under a temporary/log path.
- Keep the command safe for local dev only unless explicitly extended later.

Dependencies:

- Phase 0 holder snapshot script.
- Phase 4 normal/worker profile separation.
- Phase 6 grouped DB pressure output.
...
- `scripts/`
- `docs/workspace/dev-commands.md`
- `.logs/workspace/` or another ignored local artifact path.

Validation:

- Run the command against the default local profile.
- Confirm it exits non-zero only on real preflight/pressure failures.
- Confirm generated artifacts are redacted and ignored when appropriate.

Acceptance criteria:

- A developer can rehearse admin DB pressure before major Supabase-facing changes.
- The rehearsal produces a before/during/after holder summary.

Commit boundary:

- One workspace tooling commit after Phase 6.

### Suggestion 6 - Add owner aliases for recurring Supabase surfaces

Source: Suggestion 6, "Add owner aliases for recurring Supabase surfaces"

Concrete changes:

- Extend direct-SQL and ownership docs to support aliases such as:
  - `app-local`;
  - `admin-read-model`;
  - `backend-shared-schema`;
  - `workspace-ops`;
  - route or domain owner when known.
- Add a short alias table explaining who reviews exceptions and where the source of truth lives.
- Apply aliases while classifying the `22` unowned direct-SQL call sites.

Dependencies:

- Phase 2 ownership matrix.
- Phase 5 direct-SQL owner-label pass.
...
- `docs/workspace/env-contract.md`
- `docs/workspace/api-migration-ledger.md`
- any script output that emits owner labels.

Validation:

- `python3 scripts/app-direct-sql-inventory.py --check`
- Manual review that no touched direct-SQL row remains `needs owner label`.

Acceptance criteria:

- Recurring Supabase surfaces have stable owner aliases.
- Exceptions and migrated routes can be assigned without re-reading implementation code.

Commit boundary:

- One docs/inventory commit with Phase 5.

### Suggestion 7 - Add exception expiry dates for retained direct SQL

Source: Suggestion 7, "Add `exception expiry` dates for retained direct SQL"

Concrete changes:

- Add fields to retained direct-SQL exception rows:
  - exception owner;
  - reason retained;
  - expiry or review date;
  - migration target if known.
- Apply this to any high-fanout direct-SQL caller that cannot be migrated during Phase 5.
- Add lint/report behavior that highlights expired exceptions, or document a manual check if automation is too costly.

Dependencies:

- Phase 5 direct-SQL migration pass.
- Suggestion 6 owner aliases.

Affected surfaces:

- `docs/workspace/app-direct-sql-inventory.md`
- `docs/workspace/api-migration-ledger.md`
- `scripts/app-direct-sql-inventory.py` if it emits exception metadata.

Validation:

- `python3 scripts/app-direct-sql-inventory.py --check`
- Manual review that every retained high-fanout exception has owner, reason, and review date.

Acceptance criteria:

- Retained app direct-SQL exceptions cannot silently become permanent.
- Expired exceptions are visible to future agents/reviewers.

Commit boundary:

- One inventory/tooling commit with Phase 5.

### Suggestion 8 - Add screenshot-based smoke evidence for admin fallback UI

Source: Suggestion 8, "Add screenshot-based smoke evidence for admin fallback UI"

Concrete changes:

- Add a manual or automated smoke step that captures fallback UI evidence after Phase 8.
- Cover at least:
  - `/admin/social`;
  - one social profile or season/week social page;
  - one non-social Supabase-backed admin page.
- Capture the UI when backend reads are healthy and when one read is forced to stale/partial/failure mode.
- Store screenshots only in an ignored artifact path unless the project already has a committed visual evidence convention.

Dependencies:

- Phase 8 fallback boundaries.
- Browser/test tooling availability.

Affected surfaces:

- app smoke docs or Playwright/browser-use scripts if added;
- ignored visual evidence artifact path;
- `docs/workspace/db-pressure-runbook.md` if the evidence process is documented there.

Validation:

- Run the smoke path locally.
- Confirm screenshots show usable UI, visible stale/partial state, and no incoherent overlap or full-page crash.

Acceptance criteria:

- Operator-facing fallback behavior is visually verified, not only route-JSON verified.
- Screenshot artifacts are redacted and not committed unless intentionally documented.

Commit boundary:

- One app validation/docs commit after Phase 8.

### Suggestion 9 - Add a Supabase Dashboard evidence checklist

Source: Suggestion 9, "Add a Supabase Dashboard evidence checklist"

Concrete changes:

- Add a reusable checklist/template for Dashboard-only evidence:
  - security advisor;
  - performance advisor;
  - Auth confirmations/password policy;
  - SMTP provider;
  - Storage buckets/policies/MIME sizes;
  - connection/capacity snapshot;
  - permission blocker and owner fields.
- Use it in `docs/workspace/supabase-advisor-snapshot-YYYY-MM-DD.md` or a template file.
- Make blocked MCP access a normal field, not an excuse to leave the doc empty.

Dependencies:

- Phase 0 advisor snapshot target.
- Phase 9 production Auth/Storage/SMTP review.

Affected surfaces:

- `docs/workspace/supabase-advisor-snapshot-YYYY-MM-DD.md`
- optional `docs/workspace/supabase-dashboard-evidence-template.md`
- `docs/workspace/supabase-rls-grants-review.md`
- `docs/workspace/production-supabase-connection-inventory.md`

Validation:

- Fill one dated checklist with live values or explicit blocked-owner rows.
- Confirm no secrets are copied from the Dashboard into docs.

Acceptance criteria:

- External Supabase evidence is reproducible across future audits.
- Permission-blocked checks have owner and next action.

Commit boundary:

- One docs/template commit with Phase 0/9.

### Suggestion 10 - Add a final reviewer handoff template

Source: Suggestion 10, "Add a final reviewer handoff template"

Concrete changes:

- Add a final handoff template that summarizes:
  - changed contracts;
  - migrations applied/quarantined;
  - env behavior changes;
  - high-fanout routes migrated or excepted;
  - validation commands and results;
  - production checks completed or blocked;
  - rollback notes.
- Use it at the end of implementation before requesting review or moving to production.

Dependencies:

- All implementation phases that change code, migrations, env docs, or production posture.

Affected surfaces:

- `.plan-grader/supabase-connection-capacity-hardening-20260427-131550/`
- optional `docs/workspace/supabase-review-handoff-template.md`
- final implementation handoff under project/workspace docs if used.

Validation:

- Complete the template with actual command results and artifact links after implementation.
- Confirm it references the final source plan and any accepted exceptions.

Acceptance criteria:

- A reviewer can understand what changed and what remains risky without reconstructing the whole execution history.
- Final handoff lists every blocked production/Supabase evidence item.

Commit boundary:

- One final docs/handoff commit after implementation and verification.
