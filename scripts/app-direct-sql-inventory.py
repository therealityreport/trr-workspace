...
DEFAULT_JSON_OUTPUT = ROOT / "docs/workspace/app-direct-sql-inventory.json"
DEFAULT_EXCEPTIONS = ROOT / "docs/workspace/app-direct-sql-exceptions.json"
DEFAULT_API_LEDGER = ROOT / "docs/workspace/api-migration-ledger.json"
...
def parse_postgres_import_bindings(text: str) -> tuple[dict[str, str], set[str]]:
    ...
    namespaces = {match.group("local") for match in NAMESPACE_POSTGRES_IMPORT_RE.finditer(text)}
...
def scan_call_sites(text: str) -> list[tuple[str, int]]:
    ...
    while index < len(text):
        ...
        if char.isalpha() or char in "_$":
            ...
            if local_symbol in named_bindings and not _is_member_identifier(text, start):
                if _is_call_at(text, index):
                    calls.append((named_bindings[local_symbol], start))
                continue
            if local_symbol in namespace_bindings or local_symbol in transaction_receivers:
                member = _member_at(text, index)
                if member is None:
                    continue
                member_symbol, member_start, member_end = member
                allowed_members = CALL_SYMBOLS if local_symbol in namespace_bindings else {"query"}
...
def _iter_function_contexts(text: str, offsets: list[int]) -> Iterator[str]:
    declarations = [
        (
            match.start(),
            match.group("async_function") or match.group("function") or match.group("async_const"),
        )
    ...
    for offset in offsets:
        while declaration_index < len(declarations) and declarations[declaration_index][0] < offset:
...
def collect_uses() -> list[DirectSqlUse]:
    ...
    for path in _iter_source_files():
        ...
        for (symbol, _), line_number, context in zip(call_sites, line_numbers, contexts):
...
def _trr_shows_migration_plan(context: str) -> MigrationPlan:
    ...
    raise ValueError(f"No concrete trr-shows migration plan for function context: {context}")
...
def _migration_plan_for(use: DirectSqlUse) -> MigrationPlan:
    ...
    for marker, plan in MIGRATION_PLANS:
        if marker in path_text:
            return plan
    raise ValueError(f"No concrete migration plan for non-app-local source: {path_text}")
...
def load_exception_records(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(policy, dict) or policy.get("max_review_window_days") != MAX_REAUTHORIZATION_DAYS:
        raise ValueError(f"{path}: max_review_window_days must be {MAX_REAUTHORIZATION_DAYS}")
...
def _source_key(record: dict[str, Any]) -> tuple[str, str, int] | None:
    ...
    if not isinstance(path, str) or not path or not isinstance(symbol, str) or not symbol:
        return None
...
def _parse_record_date(record: dict[str, Any], field: str, row_id: str, errors: list[str]) -> date | None:
...
def validate_exception_records(
    ...
    *,
    ...
    sources = [source for record in records if (source := _source_key(record)) is not None]
    for source, count in sorted(Counter(sources).items()):
        if count > 1:
            errors.append(f"duplicate exception source: {source[0]}::{source[1]}::{source[2]}")
    ...
    for index, record in enumerate(records):
        ...
        if actual_source != expected_source:
            errors.append(f"{row_id}: source locator mismatch; expected {expected_source!r}")
        source = record.get("source")
        ...
        if not isinstance(execution_owner, str) or not execution_owner.strip():
            ...
            errors.append(f"{row_id}: execution_owner must be separate from durable_owner")
        ...
        if fail_expired and review_by < effective_as_of:
            errors.append(f"expired exception: {row_id} review_by={review_by} as_of={effective_as_of}")
    return errors
...
def render_markdown(uses: list[DirectSqlUse], records: list[dict[str, Any]] | None = None) -> str:
    records = records or []
...
def render_inventory_json(uses: list[DirectSqlUse], records: list[dict[str, Any]]) -> str:
...
def render_api_ledger_json(uses: list[DirectSqlUse], records: list[dict[str, Any]]) -> str:
    current = {use.row_id: use for use in uses if use.owner_alias != "app-local"}
    rows = []
    ...
    payload = {
        ...
        "counts": {
            ...
            "admin-read-model": sum(row["classification"] == "admin-read-model" for row in rows),
            "backend-shared-schema": sum(row["classification"] == "backend-shared-schema" for row in rows),
        },
        "rows": rows,
    }
    return json.dumps(payload, indent=2, ensure_ascii=False) + "\n"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--api-ledger", type=Path)
    ...
    parser.add_argument("--as-of", type=_parse_date_argument, help="Expiry evaluation date (YYYY-MM-DD).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    exceptions_path = _resolve(args.exceptions)
    ...
    if output == DEFAULT_OUTPUT:
        inventory_json = inventory_json or DEFAULT_JSON_OUTPUT
        api_ledger = api_ledger or DEFAULT_API_LEDGER
    try:
        uses = collect_uses()
        records = load_exception_records(exceptions_path)
    ...
    if errors:
        print("[app-direct-sql-inventory] ERROR: exception validation failed:", file=sys.stderr)
        for error in errors:
    ...
    outputs = [(output, rendered_markdown)]
    ...
    if args.check:
        return 0 if all(_check_output(path, expected) for path, expected in outputs) else 1
