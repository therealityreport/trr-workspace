...
WORKSPACE_TRR_JOB_PLANE_MODE="${WORKSPACE_TRR_JOB_PLANE_MODE:-local}"
WORKSPACE_TRR_LONG_JOB_ENFORCE_REMOTE="${WORKSPACE_TRR_LONG_JOB_ENFORCE_REMOTE:-0}"
WORKSPACE_TRR_REMOTE_EXECUTOR="${WORKSPACE_TRR_REMOTE_EXECUTOR:-modal}"
WORKSPACE_TRR_MODAL_ENABLED="${WORKSPACE_TRR_MODAL_ENABLED:-0}"
...
workspace_dev_mode_value() {
  ...
  echo "${WORKSPACE_DEV_MODE:-local}"
}

workspace_dev_mode_label() {
  case "$(workspace_dev_mode_value)" in
    local) echo "local" ;;
    hybrid) echo "hybrid" ;;
    cloud) echo "cloud" ;;
    *) workspace_dev_mode_value ;;
  ...
  cat <<JSON
{
  ...
  "modes": {
    ...
    "workspace_runtime_capacity_profile": "$(json_escape "$(runtime_value_or_na "${WORKSPACE_RUNTIME_CAPACITY_PROFILE:-}")")",
    "workspace_runtime_capacity_context": "$(json_escape "$(runtime_value_or_na "${WORKSPACE_RUNTIME_CAPACITY_CONTEXT:-}")")",
...
echo "  WORKSPACE_RUNTIME_CAPACITY_PROFILE: $(runtime_value_or_na "${WORKSPACE_RUNTIME_CAPACITY_PROFILE:-}")"
echo "  WORKSPACE_RUNTIME_CAPACITY_CONTEXT: $(runtime_value_or_na "${WORKSPACE_RUNTIME_CAPACITY_CONTEXT:-}")"
