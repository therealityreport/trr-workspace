...
visibility_tier() {
  local key="$1"
  case "$key" in
    WORKSPACE_OPEN_BROWSER|WORKSPACE_OPEN_SCREENALYTICS_TABS|WORKSPACE_CLEAN_NEXT_CACHE|WORKSPACE_BROWSER_TAB_SYNC_MODE|WORKSPACE_SCREENALYTICS|WORKSPACE_SCREENALYTICS_SKIP_DOCKER|WORKSPACE_SCREENALYTICS_STREAMLIT_ENABLED|WORKSPACE_SCREENALYTICS_WEB_ENABLED|WORKSPACE_TRR_JOB_PLANE_MODE|WORKSPACE_TRR_REMOTE_EXECUTOR|WORKSPACE_TRR_MODAL_ENABLED|WORKSPACE_TRR_REMOTE_WORKERS_ENABLED|WORKSPACE_TRR_REMOTE_SOCIAL_WORKERS|WORKSPACE_BACKEND_AUTO_RESTART|TRR_BACKEND_RELOAD|TRR_ADMIN_ROUTE_CACHE_DISABLED)
      echo "common"
      ;;
    WORKSPACE_*)
      echo "advanced"
...
accepted_values() {
  ...
  case "$key" in
    ...
    *TIMEOUT*|*INTERVAL*|*WORKERS|*RETRIES|*_SECONDS|*_MAX_TIME)
      echo "integer"
      ;;
    *ENABLED|*AUTO_RESTART|*SKIP_DOCKER|*FORCE_KILL_PORT_CONFLICTS|*CLEAN_NEXT_CACHE|*OPEN_BROWSER|*OPEN_SCREENALYTICS_TABS|*RELOAD|*DISABLED|*REQUIRE_REDIS_FOR_MULTI_WORKER|*LONG_JOB_ENFORCE_REMOTE|*STRICT)
      echo '`0` or `1`'
      ;;
    *)
      echo "string"
...
description_for() {
  ...
  case "$key" in
    ...
    WORKSPACE_SCREENALYTICS_STREAMLIT_ENABLED)
      echo "Enable or disable the screenalytics Streamlit UI during workspace startup."
      ;;
    WORKSPACE_SCREENALYTICS_WEB_ENABLED)
      echo "Enable or disable the screenalytics Web UI during workspace startup."
      ;;
