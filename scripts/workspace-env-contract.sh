...
PROFILE_FILE="$ROOT/profiles/local-lite.env"
...
generate_contract() {
  local out="$1"
  {
    ...
    echo
    echo 'Defaults reflect the effective `make dev` baseline (`PROFILE=local-lite`) when that profile overrides the raw script fallback.'
    echo
