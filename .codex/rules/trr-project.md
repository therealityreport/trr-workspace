...
- Short final-response note when app validation/build is relevant: `TRR-APP build: <passed | skipped, no current-chat approval | blocked: reason>.`
- For docs-only, backend-only, scraper-only, SQL-only, or Modal-only changes that do not affect TRR-APP behavior, state `TRR-APP build: not applicable (<reason>).`
...
- State app validation/build status only when app validation/build is relevant; otherwise state why TRR-APP build was not applicable.
